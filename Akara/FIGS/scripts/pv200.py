...
DIRFIG   = '/home/victor/USP/sinotica3/ATMOS-BUD_Results/Akara/FIGS/Automated_Figures/pv200_1/'
...
for i in range(n_final):
    ...
    u = ds['u'].sel(pressure_level=[250,  200, 150]) * units('m/s')
    v = ds['v'].sel(pressure_level=[250,  200, 150]) * units('m/s')
    t = ds['t'].sel(pressure_level=[250,  200, 150]) * units('kelvin')

    u = u.isel(valid_time=i)
    v = v.isel(valid_time=i)
    t = t.isel(valid_time=i)

    pressure_levels = [250,  200, 150] * units.hPa
    ...
    pot_vort = pot_vort.sel(pressure_level=200)  # Selecciona el nivel de 250 hPa
...
#)
    ...
    pv = ax.contour(lon, lat, pot_vort * 1e6, levels=np.arange(-4, -1, 1), colors='black', linewidths=2.0)
    pv1 = ax.contour(lon, lat, pot_vort * 1e6, levels=[-2], colors='blue', linewidths=1.0)
    ax.clabel(pv, inline=True, fontsize=10, fmt='%1.1f', colors='black')
